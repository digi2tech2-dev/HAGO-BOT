...
### Local operator client management

The operator-only CLI has no HTTP endpoint and never contacts Hago. It uses `MONGO_URI` and `CLIENT_API_KEY_PEPPER` from the secure server environment:
...
npm run client-admin -- revoke-key --key <key-id> --confirm
```

`list-clients`, `list-keys`, `disable-client --confirm`, `enable-client --confirm`, and `disable-key --confirm` are also available. Only `create-key` and `rotate-key` emit a newly generated full key; existing plaintext keys and key digests are never displayed.
