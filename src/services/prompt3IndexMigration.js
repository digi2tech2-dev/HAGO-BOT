...
async function inspectPrompt3Indexes(collection) {
  ...
  return { indexes, historical, targets };
}
...
async function runPrompt3IndexMigration({ collection, apply = false }) {
  ...
  return { ...plan, dryRun: false, migrated: Boolean(inspection.historical[0] || missing.length) };
}

module.exports = { TARGET_INDEXES, inspectPrompt3Indexes, findDuplicateGroups, runPrompt3IndexMigration };
