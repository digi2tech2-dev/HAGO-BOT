...
Before enabling V2 financial traffic against an existing deployment, an operator must back up MongoDB and run `npm run migrate:prompt3-indexes` as a dry run, then execute it with `-- --apply` during a maintenance window. The script checks duplicate legacy and tenant idempotency groups before it changes any index, drops only the recognized historical **unique** global `idempotencyKey_1` index or the exact redundant **non-unique sparse** single-field index produced by an earlier schema declaration, and creates the two named partial unique indexes. The application never performs this migration automatically. Until the production migration is verified, do not claim that two production clients may safely reuse the same `Idempotency-Key`. See [Multi-tenant deployment](MULTI_TENANT_DEPLOYMENT.md).
